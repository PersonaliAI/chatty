-- Durable idempotency keys for provider webhooks.
-- Meta retries the same WhatsApp delivery when the HTTP response is slow or
-- transiently unavailable. A unique provider event id prevents duplicate AI
-- replies, lead rows, and quota consumption.
...
-- Keep the idempotency table bounded. A provider will not retry an event
-- indefinitely; 90 days is enough for audit/replay investigations.
CREATE OR REPLACE FUNCTION public.prune_chatty_channel_events()
RETURNS void
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  DELETE FROM public.chatty_channel_events
