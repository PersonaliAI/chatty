...
def _process_self_host_lemon_event(
    data: dict, event_name: str, custom_data: dict, user_id: str | None,
    event_id: str, attributes: dict, variant_id: str, plan_name: str,
) -> None:
    """Apply a verified Lemon Squeezy event atomically to portable PostgreSQL.

    The managed path intentionally stays on Supabase below.  This adapter is
    idempotent on ``lemon_events.event_id`` and keeps subscription state,
    referral attribution, and commission creation in one transaction so a
    retry cannot double-charge or double-credit an affiliate.
    """
    from psycopg2.extras import Json, RealDictCursor

    now = datetime.now(timezone.utc)
    renews_at = attributes.get("renews_at") or attributes.get("ends_at")
    subscription_status = str(attributes.get("status") or "active")
    if event_name in ("subscription_cancelled", "subscription_expired"):
        subscription_status = "cancelled" if event_name == "subscription_cancelled" else "expired"
    order_id = str(attributes.get("order_id") or (data.get("data") or {}).get("id") or "")
    subscription_id = str(attributes.get("subscription_id") or attributes.get("subscription_item_id") or "")

    with connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """INSERT INTO lemon_events (event_id, event_name, raw, processed_at)
                   VALUES (%s, %s, %s, %s)
                   ON CONFLICT (event_id) DO NOTHING
                   RETURNING event_id""",
                (event_id, event_name, Json(data), now),
            )
            if cur.fetchone() is None:
                return  # Lemon retry: the first delivery already committed.

            if user_id and event_name in ("order_created", "subscription_created", "subscription_updated", "subscription_payment_success"):
                cur.execute(
                    """UPDATE users
                       SET plan = %s, subscription_status = %s,
                           subscription_renews_at = %s, subscription_variant_id = %s,
                           lemon_subscription_id = NULLIF(%s, ''), updated_at = NOW()
                       WHERE auth_user_id = %s""",
                    (plan_name, subscription_status, renews_at, variant_id, subscription_id, user_id),
                )
                cur.execute(
                    """INSERT INTO user_subscriptions (user_id, plan, variant_id, updated_at)
                       VALUES (%s, %s, %s, %s)
                       ON CONFLICT (user_id) DO UPDATE SET plan = EXCLUDED.plan,
                         variant_id = EXCLUDED.variant_id, updated_at = EXCLUDED.updated_at""",
                    (user_id, plan_name, variant_id, now),
                )
            elif user_id and event_name in ("subscription_cancelled", "subscription_expired"):
                cur.execute(
                    "UPDATE users SET subscription_status = %s, updated_at = NOW() WHERE auth_user_id = %s",
                    (subscription_status, user_id),
                )

            # Refunds void the commission generated by the matching order.
            if event_name == "order_refunded" and order_id:
                cur.execute(
                    """UPDATE affiliate_commissions
                       SET status = 'void', notes = %s, updated_at = NOW()
                       WHERE lemon_order_id = %s AND status <> 'void'
                       RETURNING referral_id""",
                    (f"Order {order_id} refunded by Lemon Squeezy (event {event_id})", order_id),
                )
                referral_ids = [row["referral_id"] for row in cur.fetchall() if row.get("referral_id")]
                if referral_ids:
                    cur.execute("UPDATE affiliate_referrals SET status = 'refunded', updated_at = NOW() WHERE id = ANY(%s)", (referral_ids,))

            if user_id and event_name in ("subscription_cancelled", "subscription_expired"):
                cur.execute("UPDATE affiliate_referrals SET status = 'cancelled', updated_at = NOW() WHERE referred_user_id = %s", (user_id,))

            referral_code = _clean_referral_code(custom_data.get("affiliate_ref") or custom_data.get("ref"))
            gross_amount_cents = _amount_cents(attributes)
            if not (user_id and referral_code and gross_amount_cents > 0 and event_name in ("order_created", "subscription_created", "subscription_payment_success")):
                return

            cur.execute(
                """SELECT id, user_id, payout_email, commission_rate_bps, payout_hold_days, status
                   FROM affiliate_profiles WHERE referral_code = %s LIMIT 1""",
                (referral_code,),
            )
            affiliate = cur.fetchone()
            if not affiliate or affiliate["status"] != "active":
                return
            buyer_email = str(attributes.get("user_email") or "").strip().lower()
            payout_email = str(affiliate.get("payout_email") or "").strip().lower()
            cur.execute("SELECT email FROM users WHERE auth_user_id = %s LIMIT 1", (str(affiliate["user_id"]),))
            affiliate_user = cur.fetchone()
            is_self_referral = (
                str(affiliate["user_id"]) == str(user_id)
                or bool(buyer_email and payout_email and buyer_email == payout_email)
                or bool(affiliate_user and buyer_email and str(affiliate_user.get("email") or "").strip().lower() == buyer_email)
            )
            if is_self_referral:
                cur.execute(
                    """INSERT INTO affiliate_fraud_flags (affiliate_id, referred_user_id, severity, reason, metadata)
                       VALUES (%s, %s, 'blocked', %s, %s)""",
                    (affiliate["id"], user_id, "Self-referral attempted (matching user ID or email address)", Json({"lemon_event_id": event_id, "event_name": event_name, "buyer_email": buyer_email})),
                )
                return

            cur.execute(
                """INSERT INTO affiliate_referrals (affiliate_id, referred_user_id, referral_code,
                       first_landing_page, utm_source, utm_medium, utm_campaign, status, converted_at, updated_at)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, 'paid', %s, %s)
                   ON CONFLICT (referred_user_id) DO UPDATE SET status = 'paid', converted_at = COALESCE(affiliate_referrals.converted_at, EXCLUDED.converted_at), updated_at = EXCLUDED.updated_at
                   RETURNING id""",
                (affiliate["id"], user_id, referral_code, custom_data.get("landing_page"), custom_data.get("utm_source") or "affiliate", custom_data.get("utm_medium"), custom_data.get("utm_campaign"), now, now),
            )
            ...
            commission_amount = round(gross_amount_cents * rate_bps / 10000)
            cur.execute(
                """INSERT INTO affiliate_commissions (affiliate_id, referral_id, referred_user_id,
                       lemon_event_id, lemon_event_name, lemon_order_id, lemon_subscription_id, currency,
                       gross_amount_cents, commission_rate_bps, commission_amount_cents, status, hold_until)
                   VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'pending',%s)
                   ON CONFLICT (lemon_event_id) DO NOTHING""",
                (affiliate["id"], referral_id, user_id, event_id, event_name, order_id, subscription_id,
                 str(attributes.get("currency") or "USD").upper(), gross_amount_cents, rate_bps, commission_amount,
                 now + timedelta(days=hold_days)),
            )
        ...
        def _record_sql() -> str:
            with connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT id, status FROM affiliate_profiles WHERE referral_code = %s LIMIT 1", (referral_code,))
                    affiliate = cur.fetchone()
                    affiliate_id = affiliate[0] if affiliate else None
                    if affiliate and affiliate[1] not in ("active", "pending"):
                        return "unavailable"
                    if ip_hash:
                        cur.execute("SELECT 1 FROM affiliate_clicks WHERE referral_code = %s AND ip_hash = %s AND created_at >= NOW() - INTERVAL '1 hour' LIMIT 1", (referral_code, ip_hash))
                        if cur.fetchone():
                            return "deduplicated"
                    cur.execute("""
                        INSERT INTO affiliate_clicks (referral_code, affiliate_id, session_id, landing_page, referrer_url,
                            utm_source, utm_medium, utm_campaign, country, ip_hash, user_agent)
                        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                    """, (referral_code, affiliate_id, _clean_text(payload.get("session_id"), 120), _clean_text(payload.get("landing_page")),
                          _clean_text(payload.get("referrer_url")), _clean_text(payload.get("utm_source"), 160), _clean_text(payload.get("utm_medium"), 160),
                          _clean_text(payload.get("utm_campaign"), 160), _clean_text(request.headers.get("cf-ipcountry") or request.headers.get("x-vercel-ip-country"), 2),
                          ip_hash, _clean_text(request.headers.get("user-agent"))))
                    return "ok"
        result = await run_db(_record_sql)
        if result == "unavailable":
            raise HTTPException(status_code=404, detail="Affiliate link unavailable")
        return {"ok": True, **({"deduplicated": True} if result == "deduplicated" else {})}
...
async def webhook_lemonsqueezy(request: Request):
    ...
    if _self_host():
        try:
            await run_db(lambda: _process_self_host_lemon_event(
                data, event_name, custom_data, user_id, event_id, attributes, variant_id, plan_name,
            ))
        except Exception:
            logger.exception("Failed to process self-host Lemon Squeezy event %s", event_id)
            raise HTTPException(status_code=500, detail="Billing webhook processing failed")
