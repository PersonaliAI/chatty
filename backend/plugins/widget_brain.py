    ...
    *,
    ...
    if scheduling_enabled:
        ...
        scheduling_block = (
            ...
            f"MANDATORY 5-STEP BOOKING & LEAD CAPTURE WORKFLOW (Strict chronological order - DO NOT skip steps):\n"
            f"1. PREFERRED DAY & TIME INQUIRY:\n"
            f"   - When the visitor asks to book a demo, meeting, or appointment, warmly confirm and ask what day and time works best for them: 'What day and time works best for you?'\n"
            ...
            f"2. AVAILABILITY CHECK & SLOT SELECTION:\n"
            f"   - {avail_instruction}\n"
            f"   - Present 2 to 4 of the returned slots using their `visitor_local_label` (which is already computed in the visitor's local timezone). State times cleanly and naturally, showing simple time aligned with the visitor's timezone and inside brackets show the timezone (e.g. 'Thursday, September 10th at 10:00 AM ({visitor_timezone or 'UTC'})' or '10:00 AM ({visitor_timezone or 'UTC'})'). NEVER output 'GMT', 'GMT+...', or raw offset strings in your responses.\n"
            f"   - If the exact requested time IS AVAILABLE:\n"
            f"     * Acknowledge that the slot is open, and proceed to Step 3.\n"
            ...
            f"   - Only after date, time, visitor real name, and visitor real email are ALL in hand:\n"
            f"     * {book_instruction}\n"
            ...
            f"     * Include the visitor's real full name in the meeting title (e.g. 'Demo Meeting with <Visitor Full Name>'). Never book with 'Demo Meeting with Guest' or placeholder names.\n"
            f"     * State the confirmed time based on the visitor's local timezone cleanly and naturally, showing simple time aligned with the visitor's timezone and inside brackets showing the timezone (e.g. 'Your meeting is scheduled for Thursday, September 10th at 10:30 AM ({visitor_timezone or 'UTC'})').\n"
            f"     * CRITICAL: Confirming availability is NOT the same as booking. Never tell the visitor a meeting is booked, scheduled, or confirmed until you have actually called the booking tool in this turn and it returned successfully.\n"
