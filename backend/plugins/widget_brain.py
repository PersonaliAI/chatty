    ...
    *,
    ...
    if scheduling_enabled:
        scheduling_block = (
            ...
            f"- Before booking any slot, you MUST collect these REQUIRED details (do not book until each is provided): {required_fields_str}, preferred date/time, and the visitor's timezone (confirm it if unclear). Also ASK for these optional details once each — they are optional, so don't block the booking if the visitor skips them: {optional_fields_str}. Ask for any missing field one at a time.\n"
